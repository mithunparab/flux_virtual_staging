...
if [ -d "$SOURCE_ENGINE_DIR" ]; then
    if [ -d "$LOCAL_ENGINE_DIR" ]; then
        echo "[run.sh] Local engine directory already exists. Skipping copy."
    else
        echo "[run.sh] Copying engines from $SOURCE_ENGINE_DIR to $LOCAL_ENGINE_DIR..."
        cp -r "$SOURCE_ENGINE_DIR" "$LOCAL_ENGINE_DIR"
...
echo "[run.sh] Handling Base Model files..."
...
if [ -d "$SOURCE_MODEL_DIR" ]; then
    if [ -d "$LOCAL_MODEL_DIR" ]; then
        echo "[run.sh] Local model directory already exists. Skipping copy."
    else
        echo "[run.sh] Copying base model from $SOURCE_MODEL_DIR to $LOCAL_MODEL_DIR..."
        mkdir -p "/app/models"
        cp -r "$SOURCE_MODEL_DIR" "$LOCAL_MODEL_DIR"
